...
static int pe_find_import(TCCState *s1, ElfW(Sym) * sym)
{
    ...
    do
    {
        ...
        // printf("find (%d) %d %s\n", n, sym_index, s);
        if (sym_index && ELFW(ST_TYPE)(sym->st_info) == STT_OBJECT && 0 == (sym->st_other & ST_PE_IMPORT) && 0 == a)
            err = -1, sym_index = 0;
...
ST_FUNC int pe_output_file(TCCState *s1, const char *filename)
{
    ...
    else if (filename)
    {
        ...
        pe.start_addr = (DWORD)((uintptr_t)tcc_get_symbol_err(s1, pe.start_symbol) - pe.imagebase);
        if (s1->nb_errors)
            ret = -1;
        else
            ret = pe_write(&pe);
